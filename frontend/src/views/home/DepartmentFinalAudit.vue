  ...
  <div class="department-final-audit-page">
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="list" border stripe>
        ...
        <el-table-column prop="evidenceUrl" label="证明材料" min-width="120">
          <template #default="{ row }">
            <el-link v-if="row.evidenceUrl" :href="row.evidenceUrl" target="_blank" type="primary">
              查看材料
            </el-link>
      ...
      <el-empty v-if="!loading && list.length === 0" description="暂无待终审的部门申报" />
    </el-card>

    <!-- 终审意见 -->
    <el-dialog
      v-model="dialogVisible"
      :title="auditStatus === 1 ? '通过终审' : '驳回终审'"
      width="500px"
    >
      <el-form>
...
async function loadList() {
  ...
  try {
    ...
    if (res.data?.code === 200) {
      ...
      ElMessage.error(res.data?.message || '获取终审列表失败')
    }
  } catch (error) {
    console.error(error)

    ElMessage.error(error?.response?.data?.message || '获取终审列表失败')
  } finally {
    loading.value = false
  }
}
...
async function submitAudit() {
  ...
  try {
    const res = await request.put(`/departmentScoreApply/final-audit/${currentApply.value.id}`, {
      status: auditStatus.value,
      reviewRemark: reviewRemark.value.trim(),
    })

    if (res.data?.code === 200) {
      ElMessage.success(auditStatus.value === 1 ? '终审通过，已计入综合测评成绩' : '终审已驳回')
      ...
      ElMessage.error(res.data?.message || '终审失败')
    }
  } catch (error) {
    console.error(error)

    ElMessage.error(error?.response?.data?.message || '终审失败')
  } finally {
